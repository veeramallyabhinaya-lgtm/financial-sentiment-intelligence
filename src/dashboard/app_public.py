...
.bp{background:#dcfce7;color:#15803d}.bn{background:#fee2e2;color:#b91c1c}.bz{background:#f1f5f9;color:#475569}
.bs{background:#dbeafe;color:#1d4ed8}.be{background:#ede9fe;color:#6d28d9}
...
<div class="hero">
  ...
  <p class="hero-sub">Real-time NLP analysis of Indian financial news &nbsp;·&nbsp; Nifty 50 constituents &nbsp;·&nbsp; 8 sectors &nbsp;·&nbsp; ET · Mint · Moneycontrol · Business Standard · RBI · SEBI</p>
</div>
...
# ── LOAD DATA (lifetime defined here) ─────────────────────────────────────────
all_articles, articles, companies, signals, lifetime = _load(lookback, min_art)

# ── LIFETIME BANNER ───────────────────────────────────────────────────────────
total_ing  = lifetime["total_ingested"]
total_sc   = lifetime["total_scored"]
uniq_src   = lifetime["unique_sources"]
uniq_co    = lifetime["unique_companies"]
last_update = all_articles[0].get("fetched_at","")[:16].replace("T"," ") if all_articles else "—"
...
<div style="display:flex;align-items:center;gap:2rem;
  ...
  <div style="text-align:center;flex:1;">
    <div style="font-family:'Syne',sans-serif;font-size:1.6rem;font-weight:800;color:#1d4ed8;line-height:1;">{total_ing:,}</div>
  ...
  <div style="text-align:center;flex:1;">
    <div style="font-family:'Syne',sans-serif;font-size:1.6rem;font-weight:800;color:#1d4ed8;line-height:1;">{total_sc:,}</div>
  ...
  <div style="width:1px;height:40px;background:rgba(147,197,253,.5);"></div>
  <div style="text-align:center;flex:1;">
    <div style="font-family:'Syne',sans-serif;font-size:1.6rem;font-weight:800;color:#1d4ed8;line-height:1;">{uniq_src}</div>
  ...
  <div style="width:1px;height:40px;background:rgba(147,197,253,.5);"></div>
  <div style="text-align:center;flex:1;">
    <div style="font-family:'Syne',sans-serif;font-size:1.6rem;font-weight:800;color:#1d4ed8;line-height:1;">{uniq_co}</div>
    ...
    with col_h:
        ...
        if hd:
            ...
            fh.update_layout(
                ...
                xaxis=dict(title=dict(text="Hour (MM-DD HH)",font=dict(size=11,color="#1e293b")),tickfont=dict(size=10,color="#1e293b"),tickangle=-30,showline=False),
                yaxis=dict(tickfont=dict(size=12,color="#1e293b"),showline=False),
                coloraxis_colorbar=dict(tickvals=[-1,0,1],ticktext=["Bearish","Neutral","Bullish"],len=0.85,thickness=14,outlinewidth=0,tickfont=dict(size=11,color="#1e293b"),title=""),
            )
...
with t2:
    ...
    if not df_co.empty:
        ...
        fig2.update_layout(
            height=max(320,len(df_co)*30),margin=dict(l=185,r=65,t=32,b=24),
            paper_bgcolor="rgba(255,255,255,0)",plot_bgcolor="rgba(255,255,255,0)",
            font=dict(family="DM Sans, sans-serif",color="#1e293b",size=12),
            xaxis=dict(range=[-1.18,1.18],tickvals=[-1,-.5,0,.5,1],ticktext=["-1.0","-0.5","0","+0.5","+1.0"],gridcolor="rgba(147,197,253,.25)",showline=False,tickfont=dict(size=11,color="#1e293b")),
            yaxis=dict(autorange="reversed",showline=False,gridcolor="rgba(147,197,253,.25)",tickfont=dict(size=12,color="#1e293b")),
            shapes=[dict(type="line",x0=0,x1=0,y0=-.5,y1=len(df_co)-.5,line=dict(color="#93c5fd",width=1,dash="dot"))],
        )
...
with t3:
    ...
    if vmode == "Single sector":
        ...
        if ts:
            ...
            f3.update_layout(height=340,margin=dict(l=80,r=28,t=32,b=72),
                paper_bgcolor="rgba(255,255,255,0)",plot_bgcolor="rgba(255,255,255,0)",
                font=dict(family="DM Sans, sans-serif",color="#1e293b",size=12),
                xaxis=dict(showticklabels=True,tickangle=-30,tickfont=dict(size=10,color="#1e293b"),gridcolor="rgba(147,197,253,.25)",showline=False),
                yaxis=dict(range=[-1.1,1.1],tickvals=[-1,-.5,0,.5,1],ticktext=["-1.0","-0.5","0","+0.5","+1.0"],
                    title=dict(text="Sentiment",font=dict(size=11,color="#1e293b")),gridcolor="rgba(147,197,253,.25)",showline=False,tickfont=dict(size=11,color="#1e293b")))
        ...
        if has_d:
            ...
            fa.update_layout(height=400,margin=dict(l=80,r=28,t=32,b=100),
                paper_bgcolor="rgba(255,255,255,0)",plot_bgcolor="rgba(255,255,255,0)",
                font=dict(family="DM Sans, sans-serif",color="#1e293b",size=12),
                xaxis=dict(showticklabels=True,tickangle=-30,tickfont=dict(size=10,color="#1e293b"),gridcolor="rgba(147,197,253,.25)",showline=False),
                yaxis=dict(range=[-1.1,1.1],tickvals=[-1,-.5,0,.5,1],ticktext=["-1.0","-0.5","0","+0.5","+1.0"],
                    title=dict(text="Sentiment",font=dict(size=11,color="#1e293b")),gridcolor="rgba(147,197,253,.25)",showline=False,tickfont=dict(size=11,color="#1e293b")),
                legend=dict(orientation="h",y=-0.22,font=dict(size=11,color="#1e293b"),bgcolor="rgba(255,255,255,.7)",bordercolor="rgba(147,197,253,.5)",borderwidth=1))
...
with t4:
    ...
    for art in filt[:show_n]:
        ...
        fetched_badge = f'<span style="font-size:.62rem;color:#2563eb;font-weight:600;background:rgba(37,99,235,.08);padding:.1rem .4rem;border-radius:5px;">📥 {fetched} UTC</span>'
